...
class Simulator():
    ...
    def __init__(self, file_name, sheet_name, Windturbine, skiprows=None, index_col=None, latitude=51.95, longitude=4.45,
                 lsm=15, terrain_factor=0.19):
    ...
    def calc_solar(self, Az=[0, 0, 0, 0], Inc=[15, 15, 15, 15], sp_area=[100, 100, 100, 100], sp_eff= 16, gref= 0):
        ...
        hai = np.round(np.sin(np.deg2rad(self.latitude)),4)*np.round(np.sin(np.deg2rad(decl)),4)+np.round(np.cos(np.deg2rad(self.latitude)),4)*np.round(np.cos(np.deg2rad(decl))*np.cos(np.deg2rad(h)),4)
        ...
        -np.sin(np.deg2rad(decl_dim))*np.cos(np.deg2rad(self.latitude))*(np.sin(np.deg2rad(beta_dim))*np.cos(np.deg2rad(gamma_dim)))
        +(np.cos(np.deg2rad(decl_dim))*np.cos(np.deg2rad(h_dim)))*np.cos(np.deg2rad(self.latitude))*np.cos(np.deg2rad(beta_dim))
        +(np.cos(np.deg2rad(decl_dim))*np.cos(np.deg2rad(h_dim)))*np.sin(np.deg2rad(self.latitude))*(np.sin(np.deg2rad(beta_dim))*np.cos(np.deg2rad(gamma_dim)))
        +np.cos(np.deg2rad(decl_dim))*np.sin(np.deg2rad(h_dim))*(np.sin(np.deg2rad(beta_dim))*np.sin(np.deg2rad(gamma_dim))))
        ...
        DTI = DHI_dim * (1-F1_dim) * (1 + np.cos(np.deg2rad(beta_dim))) / 2 + c + F2_dim * np.sin(np.deg2rad(beta_dim))
        DTI[DTI<0] = 0
